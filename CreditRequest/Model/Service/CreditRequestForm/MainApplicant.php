...
class MainApplicant extends BaseCreditRequestFormService {
    use Singleton;
    ...
    public function init() {
        ...
        $this->ReplyTypeConfig = [
			...
			////
			 self::REPLY_PAGE_TYPE_RED_AUXMONEY => [
   ...
   /*          self::REPLY_PAGE_TYPE_GREEN => [
                'getReplyBlock' => '\App\modules\CreditRequest\Block\Reply\CoApplicant\Green',
                'sendEmail' => false,
                'sendInfoNotification' => '\App\modules\CreditRequest\Block\Mail\FormReplyInfoNotification\CoApplicant\Green',
                'changeStatus' => self::STATUS_MAPPER_FOR_REPLY_PAGE_TYPES[self::REPLY_PAGE_TYPE_GREEN]
            ],
            self::REPLY_PAGE_TYPE_GREEN_AUXMONEY => [
                'getReplyBlock' => '\App\modules\CreditRequest\Block\Reply\CoApplicant\GreenAuxmoney',
                'sendEmail' => '\App\modules\CreditRequest\Block\Mail\FormReply\CoApplicant\GreenAuxmoney',
                'sendInfoNotification' => '\App\modules\CreditRequest\Block\Mail\FormReplyInfoNotification\CoApplicant\GreenAuxmoney',
                'changeStatus' => false
            ],
            self::REPLY_PAGE_TYPE_YELLOW => [
                'getReplyBlock' => '\App\modules\CreditRequest\Block\Reply\CoApplicant\Yellow',
                'sendEmail' => '\App\modules\CreditRequest\Block\Mail\FormReply\CoApplicant\Yellow',
                'sendInfoNotification' => false,
                'changeStatus' => false
            ],
            self::REPLY_PAGE_TYPE_RED => [
                'getReplyBlock' => '\App\modules\CreditRequest\Block\Reply\CoApplicant\Red',
                'sendEmail' => '\App\modules\CreditRequest\Block\Mail\FormReply\CoApplicant\Red',
                'sendInfoNotification' => false,
                'changeStatus' => self::STATUS_MAPPER_FOR_REPLY_PAGE_TYPES[self::REPLY_PAGE_TYPE_RED]
            ],
            self::REPLY_PAGE_TYPE_ZANDER_EXPORT => [
                'getReplyBlock' => '\App\modules\CreditRequest\Block\Reply\CoApplicant\Zander',
                'sendEmail' => '\App\modules\CreditRequest\Block\Mail\FormReply\CoApplicant\Zander',
                'sendInfoNotification' => false,
                'changeStatus' => self::STATUS_MAPPER_FOR_REPLY_PAGE_TYPES[self::REPLY_PAGE_TYPE_ZANDER_EXPORT]
            ], */
        ];
...
//    4 =>  'bei Eltern'
//];
            'wohnsituation' => strip_tags($formEntity['resident_type']),
            'wohnhaft_seit' => $formEntity["resident_since_year"] . '-' . $formEntity["resident_since_month"] . '-' . $formEntity["resident_since_day"],
            'miete' => ($formEntity['resident_type'] == 1) ? strip_tags($formEntity['rental_fee']) : 0,
            'eigentum' => ($formEntity['resident_type'] == 2 ) ? 1 : 0, // 2:1
            'eigentum_typ' => ($formEntity['resident_type'] == 2) ? strip_tags($formEntity['resident_owned_propery_type']) : 0,
            'eigentum_wert' => ($formEntity['resident_type'] == 2) ? strip_tags($formEntity['resident_owned_total_value_approx']) : 0,
            'eigentum_belastung' => ($formEntity['resident_type'] == 2) ? strip_tags($formEntity['resident_owned_total_load_approx']) : 0,
            'eigentum_belastung_mtl' => ($formEntity['resident_type'] == 2) ? strip_tags($formEntity['resident_owned_property_approx']) : 0,
            'eigentum_miete' => ($formEntity['resident_type'] == 2) ? strip_tags($formEntity['resident_owned_property_rental_income']) : 0,
            'eigentum_zusatz' => ($formEntity['own_residential_property'] == 1) ? 1 : 0, // 2:1
            'eigentum_typ_zusatz' => ($formEntity['own_residential_property'] == 1) ? strip_tags($formEntity['propery_type']) : 0,
            'eigentum_wert_zusatz' => ($formEntity['own_residential_property'] == 1) ? strip_tags($formEntity['total_value_approx']) : 0,
            'eigentum_belastung_zusatz' => ($formEntity['own_residential_property'] == 1) ? strip_tags($formEntity['total_load_approx']) : 0,
            'eigentum_belastung_mtl_zusatz' => ($formEntity['own_residential_property'] == 1) ? strip_tags($formEntity['property_approx']) : 0,
            'eigentum_miete_zusatz' => ($formEntity['own_residential_property'] == 1) ? strip_tags($formEntity['property_rental_income']) : 0,
            ...
            'partner_id' => (isset($_SESSION['partner_id']) && $_SESSION['partner_id']!==null) ? (int)$_SESSION['partner_id'] : 0,//($_COOKIE['partner_credicom']) ? $_COOKIE['partner_credicom'] : ''
            'subid' => ($_COOKIE['subid_credicom']) ? $_COOKIE['subid_credicom'] : '',
...
//            'status_intern' => $status_intern,  // will be set in $this->getReplyType($entityCreditRequest)
		...
		if(isset($_SESSION['partner_id']) && $_SESSION['partner_id']!==null){
			$partner_present=\App\Partner::where('partnerid',$_SESSION['partner_id'])->first();
			if(!$partner_present){
				$partnerData = (array('partnerid' => $_SESSION['partner_id'],'aktiv'=>1,'firma'=>'','name'=>'','pid'=>'','login'=>'','password'=>''));
			\App\Partner::insert($partnerData);
			}
		}
